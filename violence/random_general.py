...
if __name__ == '__main__':
    ...
    for j in range(len(tablename)):
        ...
        for i in range(N):
            ...
            data[name].append(random.choice(static_map["受教育程度"]))
            data[name].append(random.choice(static_map["刑期"]))
        ...
        data = p.reverce_score(data, reverse_order, 4)
        breeding1 = np.array([1, 4, 7, 12, 14, 19]) + 74 - 1
        breeding2 = np.array([2, 6, 9, 11, 13, 17, 21]) + 74 - 1
        breeding3 = np.array([3, 5, 8, 10, 15, 16, 18, 20]) + 74 - 1
